...
public:
    VectorLaunchWorkLoad(int numElements = 50000, int threadsPerBlock = 256) : m_numOfElements(numElements), m_threadsPerBlock(threadsPerBlock)
    {
...
int PmSamplingQueryMetrics(std::string chipName, std::vector<uint8_t> &counterAvailibilityImage, ParsedArgs &args)
{
    ...
    if (args.queryMetricProperties)
    {
        for (const auto &metricName : args.metrics)
        {
            ...
            printf("Metric Type: %s\n", metricType == CUPTI_METRIC_TYPE_COUNTER ? "Counter" : (metricType == CUPTI_METRIC_TYPE_RATIO) ? "Ratio"
                                                                                                                                      : "Throughput");
            printf("\n");
        }
        return 0;
